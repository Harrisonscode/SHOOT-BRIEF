...
function SettingsPage() {
  ...
  const uploadLogo = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    await (supabase.from("profiles") as any).update({ logo_url: path } as any).eq("id", user.id);
  ...
  const uploadAvatar = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const { error } = await (supabase.from("profiles") as any).update({ avatar_url: path }).eq("id", user.id);
  ...
  const togglePref = async (k: "dark_mode" | "default_shoot_type", v: any) => {
    if (!user) return;
    await (supabase.from("profiles") as any).update({ [k]: v } as any).eq("id", user.id);
    refreshProfile();
  };
