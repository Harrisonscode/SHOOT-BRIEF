...
import { Camera, Plus, MoreVertical, Trash2, CheckSquare, Square, X, Inbox, Star, CalendarClock, TrendingUp, Search, AlertTriangle, Repeat } from "lucide-react";
...
function Dashboard() {
  ...
  const load = async () => {
    ...
    const { data } = await (supabase.from("shoots") as any)
      .select("id,name,date,location,shoot_type,shot_list,status,editing_progress,client_name,final_delivery_date,recurrence_id")
      .eq("user_id", user.id)
  ...
  const loadCounts = async () => {
    ...
    const [{ count: bookings }, { count: reviews }] = await Promise.all([
      (supabase.from("booking_requests") as any).select("*", { count: "exact", head: true }).eq("photographer_id", user.id).eq("status", "pending"),
      (supabase.from("reviews") as any).select("*", { count: "exact", head: true }).eq("photographer_id", user.id).eq("approved", false),
    ]);
  ...
  const createNew = async () => {
    ...
    const { data, error } = await (supabase.from("shoots") as any)
  ...
  const remove = async (id: string) => {
    ...
    const { error } = await (supabase.from("shoots") as any).delete().eq("id", id);
    if (error) return toast.error(error.message);
  ...
  const deleteSelected = async () => {
    ...
    const { error } = await (supabase.from("shoots") as any).delete().in("id", Array.from(selected));
  ...
  const deleteCompleted = async () => {
    ...
    const { error } = await (supabase.from("shoots") as any).delete().in("id", completed.map((s) => s.id));
    if (error) { toast.error(error.message); return; }
  ...
  const overdueDeliveries = shoots.filter((s) => {
    ...
    const year = Number(s.final_delivery_date.split("-")[0]);
    if (isNaN(year) || year < 1900 || year > 2200) return false; // ignore malformed dates
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-4 gap-3">
        <h1 className="text-xl sm:text-2xl font-bold">Your shoots</h1>
              ...
              {shoots.length > 0 && (
                <>
                  ...
                  >
                    ...
                    <span className="sm:hidden">Clear{completedCount > 0 ? ` (${completedCount})` : ""}</span>
                  </button>
                  <button
            ...
            const cardInner = (
              <>
                ...
                <div className="mt-3 flex items-center gap-2 flex-wrap">
                  ...
                  {s.recurrence_id && (
                    <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-md bg-muted text-muted-foreground" title="Part of a recurring series">
