...
function BookingPage() {
  ...
  const submit = async () => {
    ...
    setSending(true);
    const { data: inserted, error } = await (supabase.from("booking_requests") as any).insert({
    ...
    // Fire notification emails (photographer alert + client confirmation)
    if (inserted?.id) {
      fetch("/api/private/notify/booking", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "new", booking_request_id: inserted.id }),
      }).catch(() => {});
    }
