...
import { Camera, Plus, MoreVertical, Trash2, CheckSquare, Square, X, Inbox, Star, CalendarClock, TrendingUp } from "lucide-react";
...
function Dashboard() {
  ...
  const load = async () => {
    ...
    const { data } = await supabase
      .from("shoots")
      .select("id,name,date,location,shoot_type,shot_list,status,editing_progress,client_name")
      .eq("user_id", user.id)
