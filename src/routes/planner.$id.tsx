...
function Planner() {
  ...
  const createRecurringSeries = async (config: { frequency: "weekly" | "monthly" | "custom"; intervalDays?: number; occurrences: number }) => {
    ...
    await supabase.from("shoots").update({ recurrence_id: recurrence.id } as any).eq("id", shoot.id);
    ...
    const { error: insertError } = await supabase.from("shoots").insert(newShoots as any);
...
function RepeatModal({ onClose, onConfirm, busy, shootDate, alreadyInSeries }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-16 bg-black/50 overflow-y-auto" onClick={onClose}>
      <div className="bg-card rounded-lg border shadow-lg max-w-md w-full p-5 my-4" onClick={(e) => e.stopPropagation()}>
