...
function Planner() {
  ...
  const duplicateShoot = async () => {
    ...
    if (profile && !profile.is_pro) {
      ...
      if ((count ?? 0) >= 3) { toast.error("Free plan limit reached. Upgrade to Pro for unlimited shoots."); navigate({ to: "/billing" }); return; }
    }
    setDuplicating(true);
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto pb-20">
      <div className="flex items-start justify-between gap-3">
        ...
        />
        <button onClick={duplicateShoot} disabled={duplicating} className="shrink-0 inline-flex items-center gap-2 px-3 py-2 rounded-md border bg-background hover:bg-muted text-sm disabled:opacity-60">
