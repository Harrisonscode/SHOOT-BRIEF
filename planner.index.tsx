...
function ClientPortal() {
  ...
  useEffect(() => {
    (async () => {
      const { data, error } = await supabase
        .from("shoots")
        .select(`
          id, name, date, location, shoot_type, user_id,
  ...
  const submitReview = async () => {
    ...
    const { data: inserted, error } = await (supabase.from("reviews") as any).insert({
      shoot_id: shoot.id,
      photographer_id: shoot.user_id,
    ...
    // Notify photographer
    if (inserted?.id) {
      fetch("/api/private/notify/review", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ review_id: inserted.id }),
      }).catch(() => {});
    }
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          {pct > 0 && (
            <div className="mt-8">
              ...
              {pct === 100 && <p className="text-xs font-medium mt-2" style={{ color: brand.color }}>Editing complete! 🎉</p>}
            </div>
          )}
        </div>
