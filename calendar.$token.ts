...
function CalendarPage() {
  ...
  const createOn = async (dateStr: string) => {
    ...
    if (profile && !profile.is_pro) {
      const { count } = await (supabase.from("shoots") as any).select("*", { count: "exact", head: true });
      if ((count ?? 0) >= 3) { toast.error("Free plan limit reached."); navigate({ to: "/billing" }); return; }
    }
    const { data, error } = await (supabase.from("shoots") as any).insert({ user_id: user.id, name: "Untitled Shoot", date: dateStr }).select().single() as any;
