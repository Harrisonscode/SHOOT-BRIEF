...
function InspirationPage() {
  ...
  const loadImages = async () => {
    ...
    const { data: ss } = await (supabase.from("shoots") as any).select("id,name").eq("user_id", user.id);
    setShoots((ss as any) ?? []);
  };
  ...
  const createGallery = async (name: string) => {
    ...
    const { error } = await (supabase.from("galleries") as any).insert({ user_id: user.id, name: trimmed });
  ...
  const renameGallery = async (oldName: string, newName: string) => {
    ...
    await (supabase.from("galleries") as any).update({ name: trimmed } as any).eq("user_id", user.id).eq("name", oldName);
    // Update all images in this gallery
    await (supabase.from("inspiration_images") as any).update({ gallery: trimmed } as any).eq("user_id", user.id).eq("gallery", oldName);
  ...
  const deleteGallery = async (name: string) => {
    ...
    await (supabase.from("inspiration_images") as any).update({ gallery: null } as any).eq("user_id", user.id).eq("gallery", name);
    await (supabase.from("galleries") as any).delete().eq("user_id", user.id).eq("name", name);
  ...
  const updateImage = async (id: string, patch: Partial<Img>) => {
    ...
    const { error } = await (supabase.from("inspiration_images") as any).update(patch as any).eq("id", id);
  ...
  const removeImage = async (id: string, imagePath: string, srcType: string) => {
    ...
    await (supabase.from("inspiration_images") as any).delete().eq("id", id);
  ...
  const deleteSelected = async () => {
    ...
    await (supabase.from("inspiration_images") as any).delete().in("id", Array.from(selected));
  ...
  const assignSelectedToGallery = async (galleryName: string | null) => {
    if (selected.size === 0) return;
    await (supabase.from("inspiration_images") as any).update({ gallery: galleryName } as any).in("id", Array.from(selected));
